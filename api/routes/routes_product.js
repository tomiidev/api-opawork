...
import { createProduct, getAllProducts,registersearch, getOnlyProductById,obtenerDatosDeCategoriaElegida,getProductsByProductType, updateProduct ,deleteImage, getAllImagesOfProducts,uploadImageToProduct, getSuppliers, getProductsByCategory, deleteProd, createSimpleOrder, getOrder, getOrderbyid, gProductForEdit, getDestacados, deleteimages, deleteImagep, deleteProduct, registerPayment, getCategories, changeState, moreClientsShops, addCategories, getCategoriesByUser} from '../controllers/product_controller.js';
...
router.post('/get-product-by-title/:productTitle', getOnlyProductById);
...
router.get('/more-clients-shops', moreClientsShops);
...
router.get('/get-categories-by-user', getCategoriesByUser);
router.put('/save-edits', upload.any(), updateProduct);
router.put('/change-order-state/:id', changeState);
